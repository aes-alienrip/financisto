...
public class MenuListActivity extends ListActivity {
    ...
    public static final int IMPORT_BACKUP_DATABASE_FILENAME_REQUEST_CODE = 103;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == IMPORT_BACKUP_DATABASE_FILENAME_REQUEST_CODE) {
            if (resultCode == RESULT_OK && data != null) {
                Uri fileUri = data.getData();
                if (fileUri != null) {
                    ProgressDialog d = ProgressDialog.show(this, null, this.getString(R.string.restore_database_inprogress), true);
                    new BackupImportTask(this, d).execute(String.valueOf(fileUri));
                }
            }
        }
